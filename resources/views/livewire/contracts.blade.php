    ...
    <div class="mt-0 mb-3 col-12 d-flex justify-content-end livewire-search">
        @if(!empty($customer))
            <div class="mt-2 m-r-0">
                {{Form::select('type', $typeArr, $filterContractType, ['id' => 'contractFilterType', 'class' => 'form-control status-filter', 'placeholder' => 'Select Contract Type']) }}
            </div>
        @endif
                            ...
                            <div class="contract-data">
                                ...
                                <h3 class="contract-listing-title">
                                    <span data-toggle="tooltip" title="" data-original-title="Customer"><i
                                                class="fas fa-street-view fa_fa_20px"></i>&nbsp{{ html_entity_decode($contract->customer->company_name) }}
                                    </span>
                                </h3>
                                <h3 class="contract-listing-title">
                                ...
                                @if(!empty($contract->contract_value))
                                    <h3 class="contract-listing-title">
                                        <span data-toggle="tooltip" title="" data-original-title="Contract Value"><i
                                                    class="fas fa-rupee-sign fa_fa_20px"></i>  &nbsp{{ number_format($contract->contract_value, 2) }}
                                        </span>
                                    </h3>
                                @endif
                            </div>
