...
<table class="main-table">
    <tr>
        ...
        <td class="text-right invoice-number">
            <h2 class="text-uppercase">{{ __('messages.proposal.proposal') }}</h2>
                    ...
                    <td>
                        @foreach($proposal->proposalAddresses as $address)
                            <div class="invoice-addresses text-right mb-2">
                                <p class="font-weight-bold m-0">{{ ($address->type == 1) ? __('messages.proposal.bill_to') : 
                                __('messages.proposal.ship_to') }}:</p>
                            ...
                            <table class="invoice-date-table">
                                <tr>
                                    <td class="text-right font-weight-bold"
                                        style="width: 70%;">{{ __('messages.proposal.proposal_date').': ' }}</td>
                                    <td>{{ !empty($proposal->date) ? Carbon\Carbon::parse($proposal->date)->format('jS M, Y') : 
                                        __('messages.common.n/a') }}</td>
                                </tr>
                                <tr>
                                    <td class="text-right font-weight-bold"
                                        style="width: 70%;">{{ __('messages.proposal.open_till').': ' }}</td>
                                    <td>{{ !empty($proposal->open_till) ? Carbon\Carbon::parse($proposal->open_till)->format('jS M, Y') : __('messages.common.n/a') }}</td>
                                </tr>
                                <tr>
                                    <td class="text-right font-weight-bold"
                                        style="width: 70%;">{{ __('messages.proposal.member').': ' }}</td>
                                    <td>{{ !empty($proposal->user) ? $proposal->user->full_name : __('messages.common.n/a') }}</td>
                                </tr>
                            </table>
                        </div>
                    </td>
        ...
        <td colspan="2">
            <table width="100%" class="table table-bordered invoice-sales-items mt-2">
                ...
                <tr>
                    ...
                    <th scope="col" class="text-right rate">{{ __('messages.proposal.rate') }}
                        ( <span
