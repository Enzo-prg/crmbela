        ...
        <div class="input-group">
            ...
            <div class="input-group-append">
                <div class="input-group-text">
                    <a href="#" data-toggle="modal" data-target="#addArticleGroupModal"><i class="fa fa-plus"></i></a>
                </div>
            </div>
        </div>
    </div>
    <div class="form-group col-6 col-lg-3 col-md-6 col-sm-6">
        <div class="control-label">{{ __('messages.article.internal_article').':' }}</div>
        <label class="custom-switch mt-2 pl-0">
            <input type="checkbox" name="internal_article"
                   class="custom-switch-input" {{ !empty($article->internal_article) ? 'checked' : '' }}>
    ...
    <div class="form-group col-6 col-lg-3 col-md-6 col-sm-6">
        ...
        <label class="custom-switch mt-2 pl-0">
            <input type="checkbox" name="disabled"
                   class="custom-switch-input" {{ !empty($article->disabled) ? 'checked' : '' }}>
            <span class="custom-switch-indicator"></span>
        </label>
    </div>
        ...
        <div class="row no-gutters">
            <div class="col-6">
                ...
                @if(isset($article->media[0]))
                    <a href="{{ $article->media[0]->getFullUrl() }}" target="_blank" class="attachments-view">
            ...
            <div class="col-2 mt-1 ml-1">
                <img id='previewImage' class="img-thumbnail thumbnail-preview tPreview"
                     src="{{ (isset($article->media[0])) ? $article->media[0]->getFullUrl() : asset('img/article/default-img.jpg') }}"/>
            </div>
        </div>
    </div>
