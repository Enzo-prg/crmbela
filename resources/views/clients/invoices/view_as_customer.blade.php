...
<div class="container">
    ...
    <div class="buttons d-flex mt-3 justify-content-between">
        <div class="status">
            <a href="#"
               class="btn btn-outline-secondary status-{{ \Illuminate\Support\Str::slug(\App\Models\Invoice::PAYMENT_STATUS[($invoice->payment_status)]) }}">
                {{ \App\Models\Invoice::PAYMENT_STATUS[$invoice->payment_status] }}
            </a>
        </div>
        <div class="download-btn">
            <a href="{{ route('clients.invoices.index') }}" class="btn btn-light btn-sm text-uppercase border">
                <i class="fa fa-undo"></i> {{ __('messages.common.back') }}</a>
            <a href="{{ route('clients.invoice.pdf', ['invoice' => $invoice->id]) }}"
               class="btn btn-light btn-sm text-uppercase border mx-1 {{ app()->getLocale() === 'de' || app()->getLocale() === 'fr' ? 'mobile-margin-top' : '' }}">
            ...
            @if($invoice->payment_status != \App\Models\Invoice::STATUS_PAID && $invoice->payment_status != \App\Models\Invoice::STATUS_CANCELLED)
                <a href="#" class="btn btn-success btn-sm text-uppercase mobile-margin-top" id="invoiceStripePayment">
