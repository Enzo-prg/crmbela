...
class TaskController extends AppBaseController
{
    ...
    /**
     * Display a listing of the Task. tasks.index
     *
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            return DataTables::of((new TaskDataTable())->get($request->only([
                'owner_id', 'owner_type', 'status', 'fabricante', 'priority', 'member_id',
            ])))->make(true);
        }

        $status = Task::STATUS;
        $priorities = Task::PRIORITY;
        $fabricantes = Task::FABRICANTE;
        $statusCount = $this->taskRepository->getStatusCount();

        return view('tasks.index', compact('status', 'fabricantes', 'priorities', 'statusCount'));
    }
