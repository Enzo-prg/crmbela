...
class ProposalRepository extends BaseRepository
{
     ...
     * @return mixed
     */
    public function getProposalsStatusCount($customerId = null)
    {
        if (! empty($customerId)) {
    ...
    public function getSyncList()
    {
        ...
        $data['users'] = User::orderBy('first_name', 'asc')->whereIsEnable(true)->user()->get()->pluck('full_name',
            'id')->toArray();
        $data['related'] = Proposal::RELATED_TO_array;
        $data['leads'] = Lead::orderBy('name', 'asc')->pluck('name', 'id')->toArray();
...
//        $input['phone'] = preparePhoneNumber($input, 'phone');
     ...
     * @return Proposal
     */
    public function getProposalDetailClient($proposal)
    {
        ...
        $proposal = Proposal::with([
            'user', 'salesItems.taxes', 'proposalAddresses', 'salesTaxes',
        ])->whereOwnerId($customerId)->findOrFail($proposal->id);
