...
class TaskRepository extends BaseRepository
{
    ...
    public function getStatusCount()
    {
        return Task::selectRaw('count(case when status = 1 then 1 end) as not_started')
            ...
            ->selectRaw('count(case when status = 6 then 1 end) as cancelado')
            ->selectRaw('count(case when status = 7 then 1 end) as parado')
            ->selectRaw('count(case when status = 8 then 1 end) as lead_qualificado')
            ->selectRaw('count(case when status = 9 then 1 end) as enviar_proposta')
            ->first();
    }
    ...
    public function getData()
    {
        ...
        $data['fabricante'] = Task::FABRICANTE;
    ...
    {
        $customerId = $task->getRelatedTo(Project::class, 'customer_id');
